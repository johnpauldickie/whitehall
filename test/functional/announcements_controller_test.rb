...
class AnnouncementsControllerTest < ActionController::TestCase
  ...
  view_test "index shows the date on which a speech was first published" do
    with_stubbed_rummager(@rummager) do
      ...
      @rummager.expects(:search).returns({'results' =>
                                            [{'format' => 'speech', 'id'=>'speech_id', 'public_timestamp' => first_published_at}]
                                         })
      get :index
  ...
  view_test "index shows related organisations for each type of article" do
    with_stubbed_rummager(@rummager) do
      ...
      @rummager.expects(:search).returns({'results' =>
                                            [{'format' => 'news_article', 'id'=>'news_id', 'organisations' => [{'slug' => first_org.slug}, {'slug' => second_org.slug}]},
                                             {'format' => 'speech', 'id'=>'speech_id', 'organisations' => [{'slug' => second_org.slug}]}]
                                         })
  ...
  view_test "index shows articles in reverse chronological order" do
    with_stubbed_rummager(@rummager) do
      @rummager.expects(:search).with(has_entry({:order => '-public_timestamp'})).returns({'results' =>
  ...
  view_test 'index includes tracking details on all links' do
    with_stubbed_rummager(@rummager) do
      @rummager.expects(:search).returns({'results' =>
                                            [{'format' => 'published_news_article', 'id'=>'news_id', 'link'=>'/path/to/somewhere', 'title' => 'title'}]
                                         })
