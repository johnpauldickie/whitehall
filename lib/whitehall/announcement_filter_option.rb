module Whitehall
  class AnnouncementFilterOption < FilterOption
    ...
    Statement = create(id: 5, label: "Statements", search_format_types: [SpeechType.statements.map(&:search_format_types)].flatten, edition_types: %w[Speech], speech_types: SpeechType.statements, document_type: %w(written_statement oral_statment))
